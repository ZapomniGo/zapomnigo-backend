...
router.get("/set/:id", (req, res) => {
  ...
  pool.query(
    'SELECT s.set_id, s.name, s.description, s.date_created, s.date_modified, s.category, f.term, f.definition, f.flashcard_id, u.username, rf.confidence, u.user_id FROM sets s LEFT JOIN flashcards f ON s.set_id = f.set_id INNER JOIN users u ON s.user_id = u.user_id LEFT JOIN "reviewsFlashcards" rf ON rf.set_id = s.set_id AND rf.flashcard_id = f.flashcard_id WHERE s.set_id = $1',
    [set_id],
    (err, result) => {
      if (err) {
